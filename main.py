...
class RejoindreView(discord.ui.View):
    ...
    def __init__(self, message_id, joueur1, type_pari, valeur_choisie,
                 montant):
    ...
    @discord.ui.button(label="🎯 Rejoindre le duel",
    ...
    async def rejoindre(self, interaction: discord.Interaction,
        ...
        if joueur2.id == self.joueur1.id:
            await interaction.response.send_message(
                "❌ Tu ne peux pas rejoindre ton propre duel.", ephemeral=True)
            return

        duel_data = duels.get(self.message_id)
        if duel_data is None:
            await interaction.response.send_message(
                "❌ Ce duel n'existe plus ou a déjà été joué.", ephemeral=True)
            return

        for data in duels.values():
            if data["joueur1"].id == joueur2.id or (
                    "joueur2" in data and data["joueur2"]
                    and data["joueur2"].id == joueur2.id):
                await interaction.response.send_message(
        ...
        suspense_embed.set_image(
            url=
            "https://media.licdn.com/dms/image/v2/D4E12AQFgRedt7P_4MQ/article-cover_image-shrink_600_2000/article-cover_image-shrink_600_2000/0/1691072860924?e=2147483647&v=beta&t=kuzhgDcGRovfVAsYlH1l6c3J7yfulu54arsEPldFFNI"
        )
        ...
        result_embed = discord.Embed(
            ...
            if gagnant == joueur2 else discord.Color.red())

        result_embed.add_field(
            name="👤 Joueur 1",
            value=
            f"{self.joueur1.mention}\nChoix : {EMOJIS[valeur_joueur1]} `{valeur_joueur1.upper()}`",
            inline=True)
        result_embed.add_field(
            name="👤 Joueur 2",
            value=
        ...
        result_embed.add_field(
            name="🏆 Gagnant",
            value=
            f"**{gagnant.mention}** remporte **{2 * self.montant:,} kamas** 💰",
            inline=False)
        result_embed.set_footer(
            text="🎰 Duel terminé • Bonne chance pour le prochain !")
...
@bot.tree.command(name="sleeping",
                  description="Lancer un duel roulette avec un montant.")
@is_sleeping()
...
async def sleeping(interaction: discord.Interaction, montant: int):
    ...
    for duel_data in duels.values():
        if duel_data["joueur1"].id == interaction.user.id or (
                "joueur2" in duel_data and duel_data["joueur2"]
    ...
    embed = discord.Embed(
        ...
        description=
        f"{interaction.user.mention} veut lancer un duel pour **{montant:,} kamas** 💰",
        color=discord.Color.gold())
    embed.add_field(
        name="Choix du pari",
        value=
        "Clique sur un bouton ci-dessous : 🔴 Rouge / ⚫ Noir / 🔵 Pair / 🟣 Impair",
        inline=False)
